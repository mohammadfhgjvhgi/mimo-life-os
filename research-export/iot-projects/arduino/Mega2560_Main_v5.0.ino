/* =============================================
 *  Smart Parking + Safety System  v5.0
...
void loop()
{
  ...
  // (جديد) استقبال أوامر من البايثون عبر Uno
  checkSerialCommands();
  ...
  // (جديد) إرسال البيانات للبايثون عبر Uno
  sendDataToBridge();
...
void checkSerialCommands()
{
  while (Serial1.available())
  {
    ...
    if (c == '\n')
    {
      ...
      // أرسل تأكيد الاستقبال فوراً (قبل أي delay)
      if (incomingCommand.length() > 0 && !incomingCommand.startsWith("SYSTEM:"))
      {
        Serial1.println("CMD:OK:" + incomingCommand);
      }

      if (incomingCommand == "OPEN_ENTRY")
      {
        if (!gasAlarm && !vibAlarm && !vibDoorOpenTime)
        {
          if (vehicleCount < maxCapacity && !entryNeedsRest)
          {
            Serial1.println("CMD:RUN:OPEN_ENTRY");
            handleEntry();
          }
          else if (vehicleCount >= maxCapacity)
          {
            Serial1.println("CMD:FAIL:PARK_FULL");
        ...
        {
          Serial1.println("CMD:FAIL:ALARM_ACTIVE");
        }
      }
      else if (incomingCommand == "OPEN_EXIT")
      {
        if (!gasAlarm && !vibAlarm && !vibDoorOpenTime)
        {
          if (vehicleCount > 0 && !exitNeedsRest)
          {
            Serial1.println("CMD:RUN:OPEN_EXIT");
            handleExit();
          }
        }
        else
      ...
      else if (incomingCommand == "OPEN_SECURITY")
      {
        Serial1.println("CMD:RUN:OPEN_SECURITY");
        for (int a = 0; a <= 180; a += 2) { emergencyServo.write(a); delay(15); }
      }
      else if (incomingCommand == "CLOSE_SECURITY")
      {
        Serial1.println("CMD:RUN:CLOSE_SECURITY");
        for (int a = 180; a >= 0; a -= 2) { emergencyServo.write(a); delay(15); }
      }
      else if (incomingCommand == "LIGHT_ON")
      {
        Serial1.println("CMD:RUN:LIGHT_ON");
        digitalWrite(RELAY_PIN, HIGH);
      }
      else if (incomingCommand == "LIGHT_OFF")
      {
        Serial1.println("CMD:RUN:LIGHT_OFF");
        digitalWrite(RELAY_PIN, LOW);
      }
      else if (incomingCommand == "BUZZER_ON")
      {
        Serial1.println("CMD:RUN:BUZZER_ON");
        tone(BUZZER_PIN, 1000);
      }
      else if (incomingCommand == "BUZZER_OFF")
      {
        Serial1.println("CMD:RUN:BUZZER_OFF");
        noTone(BUZZER_PIN);
      }
      else if (incomingCommand.startsWith("SYSTEM:READY"))
      {
        bridgeStatus = "CONNECTED";
        bridgeTimeout = millis();
      }
