/* Smart Parking + Safety System v7.1 - Gas Fix + LDR */
#include <Servo.h>
#include <Wire.h>
...
const int touchSensorPin = A2, ldrPin = A0;
...
void setup() {
  ...
  nightMode = (ldrValue > LDR_DARK);
  if (nightMode) digitalWrite(RELAY_PIN, HIGH);

  lcd.setCursor(1, 0);
  lcd.print("Smart Parking v7.1");
  lcd.setCursor(2, 1);
  lcd.print("+ Safety System");
  lcd.setCursor(0, 2);
  lcd.print("Touch:A2 LDR:A0");
  delay(2500);
  lcd.clear();
}
...
void loop() {
  ...
  // LDR: اذا ليل وبدون انذار → شغل الإضاءة
...
void readLDR() {
  ...
  if (!nightMode && ldrValue > LDR_DARK) {
    nightMode = true;
  } else if (nightMode && ldrValue < LDR_LIGHT) {
    nightMode = false;
    digitalWrite(RELAY_PIN, LOW);
  }
}
...
void readGasSensor() {
  ...
  bool gasNow = (smoothGas > GAS_THRESHOLD);
  if (gasNow) {
...
void handleEntry() {
  ...
  if (vehicleCount < maxCapacity) {
    ...
    if (!nightMode) digitalWrite(RELAY_PIN, LOW);
...
void updateDisplay(unsigned long currentTime) {
  ...
  String line3 = "LDR:" + String(ldrValue);
  if (nightMode) line3 += " NIGHT ";
  else line3 += " DAY   ";
